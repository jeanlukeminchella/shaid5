...
app.get(baseURL+'/campaigns/get/:ID', function(req, res){
	...
	for (i in content)
	{
		if (i.type == "campaign")
		{
			campaignsList.push(i);
		}
	}
	res.setHeader('content-type', 'application/json');
	for (nwsItem in campaignsList)
	{
		if (nwsItem.ID == ID)
		{
			res.json(nwsItem);
...
app.post('/blogs/add', function(req, res){
	...
	console.log("adding blog "+JSON.stringify(nwsItem));
	addNewsItem(nwsItem);
});
...
app.post(baseURL+'/events/add', function(req, res){
	...
	else
	{
		...
		var nwsItem =
		{
			...
			"creatorID":req.query.creatorID,
			"target":req.query.target,
			"amountDonated":req.query.amount_donated
		};
