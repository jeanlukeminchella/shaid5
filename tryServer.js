...
app.post(baseURL+'/blogs/add', function(req, res){

	var content = blogs;
	var blogsArray = content.blogs;
	...
	else
	{
		...
		if (typeof(blg.title) == "undefined"||typeof(blg.ID) == "undefined")
		{
		res.status(400);
		res.setHeader('Content-Type', 'application/json');
		res.json({"error": "not authorised, you must enter ID,title"});
		}
		else
		{
			console.log("adding event"+JSON.stringify(blg));
			blogsArray.push(blg);
			content.blogs=blogsArray;
			console.log("eventss array is now: "+JSON.stringify(blogsArray));
			blogs=content;
			res.setHeader('Content-Type', 'application/json');
			res.json({'status' : 'ok'});
		}
...
app.get(baseURL+'/blogs/get/:ID', function(req, res){


	var content = blogs;
	var blogsList = content.blogs;
	res.setHeader('content-type', 'application/json');
	for (blg in blogsList)
	{
		if (blg.ID == ID)
		{
			res.json(blg);
	...
	res.json({"error": "no such blog"});
});

app.get(baseURL+'/campaigns/get/:ID', function(req, res){


	var content = campaigns;
	var eventsList = content.events;
	res.setHeader('content-type', 'application/json');
	for (campgn in campaignsList)
	{
		if (campgn.ID == ID)
		{
			res.json(campgn);
...
app.post(baseURL+'/campaigns/donate', function(req, res){

	var content = campaigns;
	var campaignsList = content.campaigns;
	var campaignID = req.query.ID;
	var donation = parseFloat(req.query.amount);

	for (campgn in campaignsList)
	{
		if (campgn.ID == campaignID)
		{
			var currentDonation = parseFloat(campgn.amountDonated);
			var totalDonation = currentDonation + donation;
			evnt.amountDonated = totalDonation
		}
	}
	console.log("changing donation amount"+JSON.stringify(campgn));
	content.campaigns=campaignsList;
	console.log("campaign array is now: "+JSON.stringify(campaignsList));
	campaigns=content;
	res.setHeader('Content-Type', 'application/json');
	res.json({'status' : 'ok'});
});
...
app.post(baseURL+'/events/add', function(req, res){
	...
	else
	{
		...
		var evnt =
		{
			...
			"target":req.query.target,
			"amountDonated":req.query.amountDonated,
			"creatorID":req.query.creatorID
		};
...
app.post(baseURL+'/events/donate', function(req, res){
	...
	for (evnt in eventsList)
	{
		if (evnt.ID == eventID)
		{
			var currentDonation = parseFloat(evnt.amountDonated);
			var totalDonation = currentDonation + donation;
			evnt.amountDonated = totalDonation
		}
	}
	console.log("changing donated amount"+JSON.stringify(evnt));
	content.events=eventsList;
	console.log("eventss array is now: "+JSON.stringify(eventsList));
	events=content;
	res.setHeader('Content-Type', 'application/json');
	res.json({'status' : 'ok'});
});

app.post(baseURL+'/events/add2', function(req, res){
